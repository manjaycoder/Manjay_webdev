...
// Build valid URLs for icon cloud
const images = slugs.map((slug) => {
  const cleanedSlug = slug.replace(/-/g, "").toLowerCase();
  const finalSlug = fixedSlugMap[cleanedSlug] || cleanedSlug;
  return `https://cdn.simpleicons.org/${finalSlug}?color=white`;
});
...
const Skill = () => {
  ...
  return (
    <>
      <div
        className="w-full mt-10 px-4 md:px-10 relative"
        data-scroll-speed="2"
